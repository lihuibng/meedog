...
import com.macro.meedog.search.repository.EsAssetSalaryRepository;
 ...
 */
@Service
public class EsAssetSalaryServiceImpl implements EsAssetSalaryService {
    private static final Logger LOGGER = LoggerFactory.getLogger(EsAssetSalaryServiceImpl.class);
    @Resource(name = "highLevelClient")
    private RestHighLevelClient highClient;
    @Autowired
    private EsAssetSalaryRepository assetRepository;
    @Autowired
    ...
    @Override
    public Page<EsAssetSalary> search(Integer asset,
                                      Integer salary,
                                      Integer deposit,
                                      Integer other,
                                      Integer pageNum,
        ...
        return assetRepository.findByAssetOrSalaryOrOther(asset, salary, deposit, other, pageable);
    }
    ...
    public Page<EsAssetSalary> search(Integer asset,
        ...
        filterFunctionBuilders.add(new FunctionScoreQueryBuilder.FilterFunctionBuilder(QueryBuilders.matchQuery("uid", uid),
                ScoreFunctionBuilders.weightFactorFunction(10)));
        filterFunctionBuilders.add(new FunctionScoreQueryBuilder.FilterFunctionBuilder(QueryBuilders.matchQuery("asset", asset),
                ScoreFunctionBuilders.weightFactorFunction(10)));
        filterFunctionBuilders.add(new FunctionScoreQueryBuilder.FilterFunctionBuilder(QueryBuilders.matchQuery("salary", salary),
                ScoreFunctionBuilders.weightFactorFunction(10)));
        filterFunctionBuilders.add(new FunctionScoreQueryBuilder.FilterFunctionBuilder(QueryBuilders.matchQuery("deposit", deposit),
                ScoreFunctionBuilders.weightFactorFunction(10)));
        filterFunctionBuilders.add(new FunctionScoreQueryBuilder.FilterFunctionBuilder(QueryBuilders.matchQuery("other", other),
                ScoreFunctionBuilders.weightFactorFunction(10)));
        ...
        SearchHits<EsAssetSalary> searchHits = elasticsearchRestTemplate.search(searchQuery, EsAssetSalary.class);
        if (searchHits.getTotalHits() <= 0) {
            return new PageImpl<>(new ArrayList<>(), pageable, 0);
        }
        List<EsAssetSalary> searchAssetList = searchHits.stream().map(SearchHit::getContent).collect(Collectors.toList());
        return new PageImpl<>(searchAssetList, pageable, searchHits.getTotalHits());
    }

    @Override
    public Page<EsAssetSalary> recommend(Long id, Integer pageNum, Integer pageSize) {
        Pageable pageable = PageRequest.of(pageNum, pageSize);
        List<FunctionScoreQueryBuilder.FilterFunctionBuilder> filterFunctionBuilders = new ArrayList<>();
